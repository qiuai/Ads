...
class ProfileAction extends CommonAction {
    ...
    public function index(){
    	$User	=	M('Member');
    	$info	=	$User->find($_SESSION[C('WEB_AUTH_KEY')]);
    ...
    public function profileEdit(){
    	if($this->isPost()){
    		R('Admin://Member/update', array($_SESSION[C('WEB_AUTH_KEY')]));
    	}else{
    		R('Admin://Member/getMemberInfo', array($_SESSION[C('WEB_AUTH_KEY')]));
    		$this->display();
    	}
    }
    /**
    ...
    public function profilePassword(){
    	if($this->isPost()){
    		if($_REQUEST['oldpass'] && (R('Admin://Member/getPassword') == $this->pwdHash($_REQUEST['oldpass']))) {
				...
				$data['id'] = $_SESSION[C('WEB_AUTH_KEY')];
				if($Member->save($data)){
					$this->success('修改成功！');
				}
    		}else{
