...
class FinanceAction extends CommonAction {
    ...
    public function index(){
		...
		foreach($apply as $key =>$value){
			...
			$apply[$key]["pre_tax"] = $value["pre_tax"]?0:0; // 税前收入
			$apply[$key]["after_tax"] = $value["after_tax"]?0:0; // 税后收入
			$apply[$key]["fee"] = $value["fee"]?0:0; // 手续费
			$apply[$key]["amount_payable"] = $value["amount_payable"]?0:0; // 应付金额
		}
