...
class SiteWebAction extends CommonAction {
	...
	public function addCheck(){
		...
		$flag = $site->add();
		if($flag){
			$this->success('数据添加成功','SiteWeb/index');
		}else{
			$this->error("数据添加失败",'SiteWeb/site_add');
		}
	}
	...
	public function site_delete(){
		...
		$this	->success('删除成功','SiteWeb/index');
	}
	...
	public function  channel_addCheck(){
		...
		$flag = $channel->add();
		if($flag){
			$this->success('数据添加成功','SiteWeb/channel_list');
		}else{
			$this->error("数据添加失败",'SiteWeb/channel_add');
		}
	}
	...
	public function  channel_editCheck(){
		...
		$this	-> success('更改成功','SiteWeb/channel_list');
	}
	public function channel_delete(){
		$id		= $_GET["channel_id"];
		$ch 	= M("channel");
		$ch		->where("id =".$id)->delete();
		$this	->success('删除成功','SiteWeb/channel_list');
	}
}